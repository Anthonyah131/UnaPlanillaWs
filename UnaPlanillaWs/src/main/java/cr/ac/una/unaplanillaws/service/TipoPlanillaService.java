...
public class TipoPlanillaService {
    ...
    public Respuesta getTipoPlanillas() {
        try {
            ...
            for (TipoPlanilla tipoPla : tipoPlanillas) {
                TipoPlanillaDto dto = new TipoPlanillaDto(tipoPla);
                if (!tipoPla.getEmpleados().isEmpty()) {
                    List<EmpleadoDto> evaluadorDtoList = new ArrayList<>();
                    for (Empleado emple : tipoPla.getEmpleados()) {
                        EmpleadoDto empleDto = new EmpleadoDto(emple);
                        evaluadorDtoList.add(empleDto);
                    }
                    dto.getEmpleados().addAll(evaluadorDtoList);
                }
                tipoPlanillasDto.add(dto);
            }
